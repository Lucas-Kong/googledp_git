...
def _per_block_clip_grad(
    ...
    if clipping_style not in ['layer-wise','param-wise']:
        ...
        # only if it exeed the max
        config.count_total_clipping = norm_sample.numel() + config.count_total_clipping
    ...
    config.global_clipping = config.global_clipping + end - start
    config.count_clipping += stack_num